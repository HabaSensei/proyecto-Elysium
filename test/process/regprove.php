...
$areaProve= $_POST['prove-area'];

if(!$nitProve=="" && !$nameProve=="" && !$dirProve=="" && !$telProve=="" && !$webProve=="" && !$areaProve==""){
    $verificar=  ejecutarSQL::consultar("select * from proveedor where NITProveedor='".$nitProve."'");
    $verificaltotal = mysqli_num_rows($verificar);
    if($verificaltotal<=0){
        if(consultasSQL::InsertSQL("proveedor", "NITProveedor, NombreProveedor, Direccion, Telefono, PaginaWeb, CodigoArea", "'$nitProve','$nameProve','$dirProve','$telProve','$webProve','$areaProve'")){
